<div>
    <h1 class="text-3xl font-bold text-gray-800 mb-6">Menu Kategori</h1>

    @if ($hasLowStockProducts)
        <div class="mb-4 p-4 bg-yellow-100 border-l-4 border-yellow-500 text-yellow-800 rounded-md shadow">
    ...
    <div class="flex flex-wrap gap-4 mb-6">
        @foreach ($categories as $category)
            <button wire:click="updateCategory({{ $category->id }})"
                    class="px-4 py-2 rounded-xl transition font-medium {{ $selectedCategory == $category->id ? 'bg-orange-500 text-white' : 'bg-gray-200 text-gray-700 hover:bg-orange-100' }}">
                {{ $category->name }}
            </button>
        @endforeach
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div wire:key="product-{{ $item->id }}"
                ...
                <div class="flex-1">
                    ...
                    <div class="flex justify-between items-center mb-2">
                        @if ($item->discount > 0)
                            <div>
                                <span class="line-through text-red-500 text-sm">Rp{{ number_format($item->price, 0, ',', '.') }}</span><br>
                                <span class="text-green-600 font-bold text-lg">Rp{{ number_format($item->discounted_price, 0, ',', '.') }}</span>
                            </div>
                        @else
                    ...
                    @if ($item->stock <= 10 && $item->stock > 0)
                        <p class="text-sm text-yellow-600 font-semibold mb-2">⚠️ Stok hampir habis! Sisa {{ $item->stock }} produk.</p>
                    @endif
                </div>
        ...
        @empty
            <div class="col-span-full flex flex-col items-center justify-center bg-gray-100 p-8 rounded-xl shadow text-center">
                <img src="https://www.svgrepo.com/show/428431/box-package-empty.svg" alt="No products"
